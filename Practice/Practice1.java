...
public class Practice1 extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        int[] y = {50, 100, 150, 200, 250, 300}; // y coordinates for points

        for (int i = 0; i < 6; i++) {
            g2d.fillOval(x - 5, y[i] - 5, 10, 10); // Draw red data points
        }

        g2d.setColor(Color.BLUE);

        // First cluster box (without left line)
        g2d.drawLine(x, y[0], x + 40, y[0]); // top
        g2d.drawLine(x + 40, y[0], x + 40, y[1]); // right
        g2d.drawLine(x + 40, y[1], x, y[1]); // bottom
        g2d.drawLine(x + 50, (y[0] + y[1]) / 2, x + 40, (y[0] + y[1]) / 2); // middle line on right

        // Second cluster box (without left line)
        ...
        // Third cluster box (without left line)
